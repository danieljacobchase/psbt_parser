...
def parse_psbt(buffer):
    ...
    # Parse global map
    global_map = parse_map(buffer, PSBTMapType.GLOBAL)

    # Determine PSBT version
    psbt_version = -1
    for key_val in global_map.map:
        if key_val.key.key_type == 0:
            psbt_version = 0
            break
        if key_val.key.key_type == 2:
            psbt_version = 2
            break
    if psbt_version == -1:
    ...
    if psbt_version == PSBT_GLOBAL_UNSIGNED_TX: # parse transaction to determine input and output counts
        ...
        transaction = parse_transaction(transaction_buffer)
    ...
    # Parse input maps
    input_maps = [parse_map(buffer, PSBTMapType.INPUT) for _ in range(input_ct)]

    # Parse output maps
    output_maps = [parse_map(buffer, PSBTMapType.OUTPUT) for _ in range(output_ct)]
...
def parse_map(buffer, map_type: PSBTMapType):
    ...
    while peek_byte(buffer) != b'\x00':
        key_val = parse_key_val(buffer, map_type)
        map.append(key_val)
...
def parse_key_val(buffer, map_type: PSBTMapType):
    ...
    """
    key = parse_key(buffer, map_type)
    val = parse_val(buffer)
